...
impl DocumentModel {
    ...
    /// `..` escape del directorio extraído del EPUB.
    pub fn resolve_chapter_resource(&self, src: &str) -> Option<PathBuf> {
        let chapter_id = self.current_chapter_id()?;
        let chapter_path = self.core.get_resource_path(chapter_id).ok()?;
        let epub_root = normalize_path(&self.core.workdir);
    ...
    pub fn goto_toc_href(&mut self, href: &str) -> bool {
        let (file_part, _anchor) = if let Some(pos) = href.find('#') {
    ...
    pub fn chapter_base_dir(&self) -> Option<PathBuf> {
...
#[cfg(test)]
mod tests {
    use super::resolve_resource_path;
    use std::path::{Path, PathBuf};

    #[test]
    fn resolves_resource_relative_to_the_current_chapter() {
        let resolved = resolve_resource_path(
            Path::new("/book/OEBPS/Text/cap1.xhtml"),
            Path::new("/book"),
            "../Images/foto%201.jpg#preview",
        );

        assert_eq!(
            resolved,
            Some(PathBuf::from("/book/OEBPS/Images/foto 1.jpg"))
        );
    }

    #[test]
    fn rejects_resources_that_escape_the_epub_root() {
        assert_eq!(

...
use scriptorium::library_db::{LibraryDb, BookListItem};
        ...
        drop_target.connect_drop(move |_, value, _, _| {
            if let Ok(file) = value.get::<gtk::gio::File>() {
                if let Some(path) = file.path() {
                    ...
                    let reload = reload.clone();
                    glib::MainContext::default().spawn_local(async move {
                        match scriptorium::Pipeline::import_file(&db, path_str).await {
                            Ok(_) => reload(),
...
fn build_card(
    ...
    if let Some(btn_read) = btn_iter.next() {
        ...
        btn_read.connect_clicked(move |btn| {
            let mut ids = vec![book_id];
            if let Some(fb) = btn.ancestor(gtk::FlowBox::static_type()).and_downcast::<gtk::FlowBox>() {
                let selected = fb.selected_children();
                if !selected.is_empty() {
                    ids.clear();
                    for child in selected {
                        if let Ok(id) = child.widget_name().to_string().parse::<i64>() {
                            ids.push(id);
                        }
                    }
                }
            ...
            for id in ids {
                let page = crate::pages::reader::build(db.clone(), id, nav.clone());
                nav.push(&page);
            }
        });
    }
    if let Some(btn_detail) = btn_iter.next() {
        let db = db.clone();
        let nav = nav_view.clone();
        btn_detail.connect_clicked(move |btn| {
            let mut ids = vec![book_id];
            if let Some(fb) = btn.ancestor(gtk::FlowBox::static_type()).and_downcast::<gtk::FlowBox>() {
                let selected = fb.selected_children();
                if !selected.is_empty() {
            ...
            for id in ids {
                let page = crate::pages::detail::build(db.clone(), id, nav.clone());
                nav.push(&page);
            }
        });
    }
    if let Some(btn_delete) = btn_iter.next() {
        ...
        btn_delete.connect_clicked(move |btn| {
            let mut ids = vec![book_id];
            let mut weaks = vec![card_weak.clone()];
            if let Some(fb) = btn.ancestor(gtk::FlowBox::static_type()).and_downcast::<gtk::FlowBox>() {
                let selected = fb.selected_children();
                if !selected.is_empty() {
                    ids.clear();
                    ...
                    for child in selected {
                        if let Ok(id) = child.widget_name().to_string().parse::<i64>() {
                            ...
                            if let Some(c) = child.child() {
                                if let Ok(b) = c.downcast::<gtk::Box>() {
                                    weaks.push(b.downgrade());
                                }
                            }
                        }
                    }
            ...
            show_delete_dialog(btn, db.clone(), ids, weaks);
        });
    }
        ...
        .build();
    {
        ...
        gesture.connect_pressed(move |gesture, _, _, _| {
            ...
            popover.set_parent(&gesture.widget().unwrap());
            popover.popup();
    ...
    child.set_widget_name(&book_id.to_string());
...
fn show_delete_dialog(
    ...
    db: LibraryDb,
    book_ids: Vec<i64>,
    card_weaks: Vec<glib::WeakRef<gtk::Box>>,
) {
    let parent = btn.root().and_downcast::<gtk::Window>();
    let msg = if book_ids.len() > 1 {
        format!("¿Eliminar {} libros?", book_ids.len())
    } else {
        "¿Eliminar libro?".to_string()
    };
    let dialog = gtk::AlertDialog::builder()
        .message(&msg)
        .detail("Los registros se eliminarán de la biblioteca. Los archivos físicos no se borrarán.")
    ...
    dialog.choose(
        ...
        move |result: Result<i32, glib::Error>| {
            if result.unwrap_or(-1) == 1 {
                let db = db.clone();
                let card_weaks = card_weaks.clone();
                glib::MainContext::default().spawn_local(async move {
                    for id in book_ids {
                        if let Err(e) = db.delete_book(id, false).await {
                            eprintln!("Error eliminando libro: {e}");
                        }
                    }
                    for cw in card_weaks {
                        if let Some(card) = cw.upgrade() {

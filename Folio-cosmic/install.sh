...
APP_ID="com.gutenreader.GutenReaderCosmic"
PROJECT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
DESKTOP_SOURCE="$PROJECT_DIR/$APP_ID.desktop"
DEFAULT_ICON_SOURCE="$PROJECT_DIR/icono-folio.png"
ICON_SOURCE="$DEFAULT_ICON_SOURCE"
RELEASE_BIN="$PROJECT_DIR/target/release/folio-cosmic"
BIN_DIR="${HOME}/.local/bin"
APPLICATIONS_DIR="${XDG_DATA_HOME:-${HOME}/.local/share}/applications"
ICON_ROOT="${XDG_DATA_HOME:-${HOME}/.local/share}/icons/hicolor"
INSTALLED_BIN="$BIN_DIR/folio-cosmic"
...
build_release() {
    ...
    echo "Compilando Folio en modo release..."
    CARGO_TARGET_DIR="$PROJECT_DIR/target" cargo build \
        --manifest-path "$PROJECT_DIR/Cargo.toml" \
        --package folio-cosmic \
        --release \
        --locked
}

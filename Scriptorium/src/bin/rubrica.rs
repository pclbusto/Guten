...
use scriptorium::{Analytics, LibraryDb, Organizer, Pipeline, SyncSubsystem};
use scriptorium::pipeline::ImportStatus;
use rustyline::completion::{Completer, Pair};
use rustyline::{Config, Editor};
use colored::*;
use std::path::PathBuf;
use scriptorium_pdf_pipeline::PipelineStatus;
...
async fn main() -> Result<()> {
    let cli = Cli::parse();
    let db_str = cli.db.unwrap_or_else(|| scriptorium::default_db_url());
...
async fn execute(db_url: &str, cmd: Commands) -> Result<()> {
    match cmd {
        ...
        Commands::ImportPdf { path, title, lang, output_dir, use_ocr } => {
            ...
            let config = scriptorium_pdf_pipeline::PipelineConfig::default();
            if use_ocr {
            ...
            let pipeline = scriptorium_pdf_pipeline::PdfPipeline::new(config);
        ...
        Commands::ExportConfig { path } => {
            ...
            let mut config = scriptorium::RubricaConfig::default();
            for a in aliases {
        ...
        Commands::ImportConfig { path } => {
            ...
            let config = scriptorium::RubricaConfig::from_toml(&contents)?;
            let mut count = 0;
...
fn print_books_table(books: &[scriptorium::library_db::BookListItem]) {
    let tw = term_width();
...
fn print_books_vertical(books: &[scriptorium::library_db::BookListItem]) {
    for b in books {
        let series = b.series_name.as_deref().unwrap_or("-");
        let norm = if b.is_normalized { "Sí".green() } else { "No".red() };
...
async fn print_books_extralong(books: &[scriptorium::library_db::BookListItem]) -> Result<()> {
    for b in books {
        let series = b.series_name.as_deref().unwrap_or("-");
...
fn print_authors_table(authors: &[scriptorium::library_db::AuthorStats]) {
    let tw = term_width();
...
fn print_authors_vertical(authors: &[scriptorium::library_db::AuthorStats]) {
    for a in authors {
...
fn print_series_table(series_list: &[scriptorium::library_db::SeriesStats]) {
    let tw = term_width();
    let fixed = 5 + 1 + 7; // ID + sep + Libros
    let available = tw.saturating_sub(fixed).max(20);
    let name_w = available.max(10);
...
fn print_series_vertical(series_list: &[scriptorium::library_db::SeriesStats]) {
    for s in series_list {
